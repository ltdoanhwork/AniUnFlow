...
\subsection{Segment Anything Model as Structural Prior}
The Segment Anything Model (SAM)~\cite{sam} marked an important shift in visual representation learning by showing that a single promptable segmentation model can generalize across a very wide range of objects and scenes. Instead of treating segmentation as a narrowly defined closed-set recognition task, SAM was trained to produce high-quality masks under flexible prompting and at broad scale. This matters for correspondence problems because a good segmentation model offers more than object outlines: it exposes coherent regions, part boundaries, thin structures, and layered silhouettes that are often difficult to recover from local texture alone. The more recent SAM~2 formulation~\cite{ravi2024sam2} further reinforces this idea by extending the same foundation-model perspective toward image and video settings, strengthening the view that segmentation can serve as a reusable structural interface rather than only a standalone end task.

For this thesis, SAM is relevant not because the goal is segmentation itself, but because animation optical flow benefits greatly from reliable region structure. Anime imagery often contains large flat-color interiors, repeated colors across distinct objects, and sharp line-defined boundaries. In such scenes, dense correspondence estimated only from local appearance can become unstable or underconstrained. SAM-derived masks provide a complementary cue: they partition the frame into motion-relevant regions that help the model reason about object extent, contour continuity, and part-level grouping before dense flow refinement begins. In other words, SAM gives AniUnFlow a structure-first view of the scene, which is especially valuable when ordinary photometric matching is weak.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{img/sam_structural_prior_overview.png}
    \caption{\textbf{Why SAM is useful as a structural prior for AniUnFlow.}
    A representative animation frame from AnimeRun is shown together with its contour map, the corresponding SAM region partition, and a boundary-aware overlay. The visual point is that SAM reorganizes flat-color animation imagery into coherent parts and explicit boundaries before dense motion estimation begins. This is the role SAM plays in the thesis: not motion supervision, but scene structure that stabilizes slot formation, boundary reasoning, and later flow refinement.}
    \label{fig:sam_structural_prior}
\end{figure*}

At the same time, the thesis does not treat SAM masks as motion supervision or as temporally perfect annotations. Off-the-shelf segmentation may be noisy, over-fragmented, or inconsistent across adjacent frames, especially under stylized deformation and extreme pose change. This is precisely why AniUnFlow uses SAM as a \emph{structural prior} rather than as a target to imitate. The masks organize slot pooling, inform boundary-sensitive refinement, and support propagated agreement analysis, but the final correspondence still emerges from unsupervised motion learning over the video itself. This design choice keeps the framework aligned with the thesis objective: using SAM to stabilize motion reasoning in animation, without collapsing the optical-flow problem into a segmentation problem.

In this work, we build on these insights and target optical flow estimation specifically for animation scenarios. Our method is designed to better exploit multi-frame temporal structure and global correspondence cues, with the goal of handling stylized appearance, sparse local texture, long-range motion, and persistent occlusion more effectively than approaches transferred directly from natural-video benchmarks. Internally, the development path of AniUnFlow moved from early hybrid dense/SAM designs to a unified object-memory framework with stronger propagation and large-motion reasoning, but the thesis deliberately presents the final system under one shared framework name rather than as a sequence of separately emphasized versions.
