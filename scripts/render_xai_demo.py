...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render interpretability/XAI figures for AniUnFlow.")
    parser.add_argument(
        "--config",
        default=str(ROOT / "workspaces" / "v5_4_sam_propagation_memory" / "config.yaml"),
        help="Path to AniUnFlow config YAML.",
    )
    parser.add_argument(
        "--checkpoint",
        default=str(ROOT / "workspaces" / "v5_4_sam_propagation_memory" / "best.pth"),
        help="Path to AniUnFlow checkpoint.",
    )
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "reports" / "img" / "aniunflow_xai"),
        help="Directory for rendered XAI panels.",
    )
    parser.add_argument(
        "--scene",
        default="cami_02_05_A",
        help="Scene name under AnimeRun test split.",
    )
    parser.add_argument(
        "--centers",
        nargs="*",
        type=int,
        default=[31, 43],
        help="Frame centers used for the XAI panels.",
    )
    parser.add_argument(
        "--data-root",
        default=None,
    ...
    parser.add_argument(
        "--fp32",
        action="store_true",
        help="Disable autocast and run inference in fp32.",
    )
...
def choose_scene(dataset: UnlabeledClipDataset, requested_scene: str) -> Tuple[int, Dict[str, Any]]:
...
def draw_labeled_tile(image_np: np.ndarray, label: str, tile_size: Tuple[int, int]) -> Image.Image:
    ...
    image = image.resize(tile_size, Image.Resampling.BILINEAR)
    ...
    draw.text((18, 14), label, fill=(255, 255, 255, 255), font=font)
    return canvas
...
def render_case_panels(
    ...
    slot_flow = resize_flow_tensor(output["slot_flow_fw"][pair_idx][0].detach().cpu(), tuple(gt_flow.shape[-2:]))
    dense_flow = resize_flow_tensor(output["dense_prior_flow_fw"][pair_idx][0].detach().cpu(), tuple(gt_flow.shape[-2:]))
    corr_conf = resize_map_tensor(output["corr_confidence_fw"][pair_idx][0].detach().cpu(), tuple(gt_flow.shape[-2:]))
    ...
    rad_inputs = [
        ...
        pred_flow.permute(1, 2, 0).numpy(),
        slot_flow.permute(1, 2, 0).numpy(),
        dense_flow.permute(1, 2, 0).numpy(),
    ]
    ...
    sam_memory_map = sam_memory[0].numpy()
    corr_conf_map = corr_conf[0].numpy()
    disagreement_map = 1.0 - np.clip(sam_memory_map, 0.0, 1.0)

    epe_mean = float(epe_map.mean())
    residual_mean = float(residual_mag.mean())
    gt_motion_mean = float(torch.norm(gt_flow, dim=0).mean().item())
    ...
    detailed_tiles = [
        ...
        ("Dense prior", flow_to_rgb(dense_flow, rad_max)),
        ("Final flow", flow_to_rgb(pred_flow, rad_max)),
        ("GT flow", flow_to_rgb(gt_flow, rad_max)),
        ("SAM agreement", float_map_to_rgb(sam_memory_map, getattr(cv2, "COLORMAP_VIRIDIS", cv2.COLORMAP_JET), 0.0, 1.0)),
        ("Corr confidence", float_map_to_rgb(corr_conf_map, getattr(cv2, "COLORMAP_VIRIDIS", cv2.COLORMAP_JET), 0.0, 1.0)),
    ...
    detail_path = output_dir / "panels" / f"{scene_name}_{frame_tag}_xai_detailed.png"
    focus_path = output_dir / "panels" / f"{scene_name}_{frame_tag}_xai_focused.png"

    build_panel(
        detailed_tiles,
        title=f"{scene_name} | frame {frame_tag} | explanation anatomy",
        subtitle=f"mean EPE={epe_mean:.2f}px | mean GT motion={gt_motion_mean:.2f}px | mean residual={residual_mean:.2f}px",
    ...
    build_panel(
        focused_tiles,
        title=f"{scene_name} | frame {frame_tag} | confidence-disagreement view",
        subtitle="The focused view isolates where structure remains trustworthy, where the model hesitates, and where residual correction becomes active.",
    ...
    return {
        "frame": frame_tag,
        "center": center,
        "detail_panel": str(detail_path.relative_to(ROOT)),
        "focus_panel": str(focus_path.relative_to(ROOT)),
        "mean_epe": epe_mean,
...
def main() -> None:
    ...
    for center in args.centers:
        if center < temporal_radius or center >= len(seq["frames"]) - temporal_radius:
            raise ValueError(f"Center {center} is invalid for scene '{scene_name}' and clip length {len(seq['frames'])}.")
        ...
        record = render_case_panels(model, sample, scene_name, center, output_dir, device, use_autocast)
        records.append(record)
        detail_paths.append(ROOT / record["detail_panel"])
        focus_paths.append(ROOT / record["focus_panel"])
        print(f"[XAI] Saved {record['detail_panel']}")
        print(f"[XAI] Saved {record['focus_panel']}")

    detail_sheet = output_dir / f"{scene_name}_xai_sheet.png"
    focus_sheet = output_dir / f"{scene_name}_xai_focus_sheet.png"
    build_contact_sheet(detail_paths, detail_sheet)
    build_contact_sheet(focus_paths, focus_sheet)
    print(f"[XAI] Saved {detail_sheet.relative_to(ROOT)}")
    print(f"[XAI] Saved {focus_sheet.relative_to(ROOT)}")
    ...
    print(f"[XAI] Saved {metadata_path.relative_to(ROOT)}")
