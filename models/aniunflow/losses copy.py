...
class UnsupervisedFlowLoss:
    ...
    def photometric_loss(self, frames: torch.Tensor, flows: List[torch.Tensor], alpha_ssim: float = 0.2) -> torch.Tensor:
        ...
        for k, f in enumerate(flows):
            ...
            f_full = F.interpolate(f, size=(H, W), mode='bilinear', align_corners=True)
            sx = W / f.shape[-1]
            sy = H / f.shape[-2]
            f_full[:, 0] *= sx
            f_full[:, 1] *= sy
    ...
    def temporal_composition(self, flows: List[torch.Tensor], w: float = 0.05) -> torch.Tensor:
        """
        Thay vì ép F(t->t+2) = 0, ta ép: F(t->t+1) ≈ warp(F(t+1->t+2), F(t->t+1))
        => motion mượt, không bị giật.
        """
        loss = 0.0
        count = 0
        for k in range(len(flows) - 1):
            f01 = flows[k]      # Bx2xhxw
            f12 = flows[k+1]    # Bx2xhxw

            # đưa F12 về frame t bằng cách warp theo F01
            f12_w = warp(f12, f01)

            # khuyến khích "vận tốc gần như không đổi theo thời gian"
            vel_diff = (f01 - f12_w).abs().mean()
            loss = loss + vel_diff
            count += 1
