...
\end{equation}
where $\mathbf{B}_t$ is the SAM-derived boundary map and $\mathbf{Q}_t$ is a confidence map obtained by averaging slot confidence and dense-correlation confidence. The final flow is
\begin{equation}
\mathbf{F}_{t\rightarrow t+1} = \mathbf{F}^{\mathrm{dense}}_{t\rightarrow t+1} + \mathbf{R}_{t\rightarrow t+1}.
\end{equation}
...
\label{eq:total_loss}
\begin{aligned}
\mathcal{L} = {}& \mathcal{L}_{\mathrm{unsup}} + \lambda_{\mathrm{sw}}\mathcal{L}_{\mathrm{segwarp}} + \lambda_{\mathrm{pr}}\mathcal{L}_{\mathrm{piece}} \\
& + \lambda_{\mathrm{ds}}\mathcal{L}_{\mathrm{dense-slot}} + \lambda_{\mathrm{cyc}}\mathcal{L}_{\mathrm{cycle}} + \lambda_{\mathrm{ord}}\mathcal{L}_{\mathrm{order}} \\
& + \lambda_{\mathrm{br}}\mathcal{L}_{\mathrm{boundary}} + \lambda_{\mathrm{dist}}\mathcal{L}_{\mathrm{distill}}.
\end{aligned}
\end{equation}

The term $\mathcal{L}_{\mathrm{unsup}}$ follows the standard bidirectional unsupervised optical flow recipe based on photometric reconstruction, smoothness, forward--backward consistency, and occlusion-aware masking \cite{unflow}. On top of this, we add segment warp consistency,
\begin{equation}
...
The dense-slot term in AniUnFlow-SamT regularizes the dense prior to stay close to the slot-affine motion within confident object interiors:
\begin{equation}
...
\end{equation}
Finally, following self-distillation ideas in unsupervised flow learning \cite{ddflow}, we maintain an exponential moving average teacher and penalize disagreement between student and teacher flows in confident regions. The teacher is activated only after an initial warm-up period so that early unstable predictions are not reinforced.

We employ a staged curriculum to progressively increase difficulty. During the initial stage, the model is trained on stride-$1$ clips without the residual branch, which forces learning to concentrate on object correspondence and affine motion. In the second stage, we expand the temporal stride range and enable both the dense residual branch and layered-order regularization. In later stages, we enable long-gap photometric consistency and segment-cycle losses, and finally increase the maximum clip stride to expose the model to larger displacement.

For the main configuration, we use: $T=5$, $S=32$ object slots, encoder width $40$, slot dimension $160$, temporal memory depth $3$, level-2 correlation radius $6$, level-1 correlation radius $4$, and a dense-prior fusion weight $\alpha=0.72$. The residual refiner uses $128$ hidden channels and $4$ residual-style convolutional blocks. Optimization is performed with AdamW, OneCycle learning-rate scheduling, mixed precision, gradient clipping, and an EMA teacher. These settings follow the practical observation that unsupervised optical flow benefits from strong optimization control and staged training, especially when multiple regularizers are activated progressively \cite{unflow,ddflow}.

Taken together, the method follows a simple progression. SAM first organizes the scene into coherent motion units; temporal memory stabilizes those units over the clip; layered affine reasoning converts them into a structured motion prior; dense correlation restores large and non-rigid motion detail; and the final boundary-aware refiner concentrates corrective capacity where dense motion estimation remains hardest. This is the central principle of the model: object structure should organize motion estimation, and dense correspondence should refine it rather than replace it.
